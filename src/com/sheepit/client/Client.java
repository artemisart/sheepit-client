...
public class Client {
	...
	protected Error.Type runRenderer(Job ajob) {
		...
		if (ajob.getUseGPU() && this.config.getGPUDevice() != null) {
			core_script = String.format(core_script + "bpy.context.user_preferences.system.compute_device = '%s'\n", "CUDA", "GPU", this.config.getGPUDevice().getCudaName());
		}
		else {
			core_script = String.format(core_script, "NONE", "CPU");
		}
